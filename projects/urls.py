...
urlpatterns = [
    ...
    url(r'^(?P<pk>[0-9]+)/add-image/$', login_required(ProjectScreenShotCreateView.as_view()), name='screenshot_create'),
    url(r'^(?P<pk>[0-9]+)/gallery/$', ProjectScreenShotListView.as_view(), name='screenshot_list'),
    ...
    url(r'^image/(?P<pk>[0-9]+)/delete/$', login_required(ProjectScreenShotDeleteView.as_view()), name='screenshot_delete'),
]
